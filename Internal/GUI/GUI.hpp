...
namespace GUI
{
	...
	inline std::unique_ptr<SidebarButton> GuiAllFeaturesSidebarButton = std::make_unique<SidebarButton>(std::string("ALL_FEATURES_BUTTON"), "ALL_FEATURES_BUTTON"Hashed, ElementBase::Style_t{
		.vec2Size = ImVec2(-0.1f, 0) }, ICON_FA_LIST, true);
	inline std::unique_ptr<SidebarButton> GuiDeveloperSidebarButton = std::make_unique<SidebarButton>(std::string("DEVELOPER_BUTTON"), "DEVELOPER_BUTTON"Hashed, ElementBase::Style_t{
		.vec2Size = ImVec2(-0.1f, 0) }, ICON_FA_TERMINAL, true);
	inline std::unique_ptr<SidebarButton> GuiConfigSidebarButton = std::make_unique<SidebarButton>(std::string("CONFIG_BUTTON"), "CONFIG_BUTTON"Hashed, ElementBase::Style_t{
	...
	inline std::unique_ptr<HeaderGroup> GuiHeaderGroup = std::make_unique<HeaderGroup>(std::string("HEADER_GROUP"), "HEADER_GROUP"Hashed, ElementBase::Style_t{
		.vec2Size = ImVec2(-0.1f, 0), }, std::vector<ElementBase::Header_t>{});

	// Body (Relies on Header for subpages)
	inline std::unique_ptr<Body> GuiBody = std::make_unique<Body>(std::string("BODY"), ElementBase::Style_t{
		.vec2Size = ImVec2(-0.1f, 0), });

	// Pages
	inline std::unique_ptr<Page> GuiAllFeaturesPage = std::make_unique<Page>("ALL_FEATURES_PAGE", ElementBase::Style_t(), 0, 0);
