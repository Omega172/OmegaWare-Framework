...
namespace Utils {
	void LogHook(const std::string& sHookName, const Hooking::HookStatus eStatus, std::source_location location)
	{
	#ifdef _DEBUG
		if (eStatus == Hooking::HookStatus::Installed) {
		...
		std::cout << std::format("{}Hook[{}]{}: {} {}Failure{}: {}\n", colors::cyan, sHookName, colors::white, GetLocationString(location), colors::yellow, colors::white, Hooking::ToString(eStatus));
	#endif
	}
